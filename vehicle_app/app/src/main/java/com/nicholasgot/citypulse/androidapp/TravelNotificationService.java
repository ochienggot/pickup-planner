...
public class TravelNotificationService extends Service {
	...
	private final class NotificationHandler extends Handler implements
		...
		public void handleMessage(Message msg) {
				...
				public void onOpen(Session session, EndpointConfig config) {
					...
					session.addMessageHandler(new MessageHandler.Whole<String>() {
						...
						public void onMessage(String message) {
							...
							System.out.println("Received contextual event "
									+ message);
			...
			try {
				...
				System.out
						.println("Connecting to contextual filtering for travel notification "
								+ requestEndpoint);
				...
				while (!stopThread) {
					...
					if (lastLocation != null) {
						...
						// System.out.println("Coordonate GPS:" + new
						// Gson().toJson(userGPSCoordinate));
				...
				System.out
						.println("Unable to open the connection with contextual filtering for travel  Deployment exception");

				Intent intent = new Intent();
				intent.setAction(DefaultValues.ERROR_MESSAGE);
				intent.putExtra(DefaultValues.ERROR_MESSAGE_PAYLOAD,
						"Unable to connect to the contextual filtering for travel module: "
								+ requestEndpoint);
				sendBroadcast(intent);
				...
				// TODO Auto-generated catch block
				System.out
						.println("Unable to open the connection with contextual filtering for travel IO exception");
				e.printStackTrace();
				Intent intent = new Intent();
				intent.setAction(DefaultValues.ERROR_MESSAGE);
				intent.putExtra(DefaultValues.ERROR_MESSAGE_PAYLOAD,
				...
				// TODO Auto-generated catch block
				System.out
						.println("Unable to open the connection with contextual filtering for travel URI sintax error");
				e.printStackTrace();
				Intent intent = new Intent();
				intent.setAction(DefaultValues.ERROR_MESSAGE);
				intent.putExtra(DefaultValues.ERROR_MESSAGE_PAYLOAD,
				...
				System.out
						.println("Unable to open the connection with contextual filtering for travel URI InterruptedException");
				e.printStackTrace();
				Intent intent = new Intent();
				intent.setAction(DefaultValues.ERROR_MESSAGE);
				intent.putExtra(DefaultValues.ERROR_MESSAGE_PAYLOAD,
				...
				intent.putExtra(
						...
						+ "the app was connected to the server for receiving traffic events. Most probably the "
